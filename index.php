...
class v2Ray
{
  ...
  function __construct($host, $port, $rootPath, $username, $password)
  {
    $this->host = $host;
    $this->port = $port;
  ...
  private function login()
  {
    ...
    curl_setopt_array(
      $curl,
      array(
        CURLOPT_URL => 'http://' . $this->host . ':' . $this->port . '/' . $this->rootPath . '/login',
  ...
  public function users()
  {
    ...
    curl_setopt_array(
      $curl,
      array(
        CURLOPT_URL => 'http://' . $this->host . ':' . $this->port. '/' . $this->rootPath . '/xui/inbound/list',
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => '',
        CURLOPT_MAXREDIRS => 10,
        CURLOPT_COOKIEFILE => './.cookie.txt',
  ...
  public function delete($uid)
  {
    ...
    curl_setopt_array(
      $curl,
      array(
        CURLOPT_URL => 'http://' . $this->host . ':' . $this->port. '/' . $this->rootPath . '/xui/inbound/del/' . $uid,
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => '',
        CURLOPT_MAXREDIRS => 10,
        CURLOPT_COOKIEFILE => './.cookie.txt',
  ...
  {
    if ($remark == null)
      $remark = $this->RandomString();
    if ($port == null)
      $port = rand(11111, 99999);
    $traffic = $traffic * 1024 * 1024 * 1024;
    $guidv4 = $this->guidv4();
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, 'http://' . $this->host . ':' . $this->port . '/' . $this->rootPath. '/xui/inbound/add');
